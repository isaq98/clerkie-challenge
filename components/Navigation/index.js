'use client';
import "./_Navigation.css";
import TopMenu from "../TopMenu";
import { useState, useEffect } from 'react';
import { mockNavigationObject } from "@/constants/enums";

function Navigation(props) {
    const { setRouteCallback } = props;
    const [pageName, setPageName] = useState("Home");

    useEffect(() => {
        setRouteCallback(pageName);
    }, [pageName])

    const renderSidebarButtons = () => {
        return (
            <ul className="sidebar-navigation-list">
                {mockNavigationObject.map((element, idx) => 
                <li key={`${element}-${idx}`} 
                    className={`sidebar-button ${pageName === element.title ? "active" : ""}`} 
                    onClick={() => setPageName(element.title)}>
                    {element.svg}
                    <h4>{element.title}</h4>
                </li>)}
            </ul>
        )
    }

    return (
        <>
        <TopMenu pageName={pageName} />
        <div className="sidebar-container">
                <div className="sidebar-header">
                    <div className="sidebar-header-content">
                        <svg className="clerkie-svg" aria-label="company-logo" width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M20 10.0087C20 15.5381 15.4853 20.0087 9.97059 20.0087C8.11765 20.0087 6.39706 19.5087 4.91177 18.6263C4.76471 18.5969 4.52941 18.6557 4.42647 18.6851C3.85294 18.8175 3.17647 19.0087 2.91176 19.0822C1.97059 19.244 1.51471 19.1704 1.26471 19.0087C1.05882 18.8763 1.10294 18.7146 1.11765 18.6851C1.14706 18.5969 1.19118 18.5675 1.26471 18.4793C1.36765 18.3763 1.54412 18.244 1.83824 17.8469C2.22059 17.2587 2.30882 16.6116 2.29412 16.3469C0.852941 14.6263 0 12.4057 0 10.0087C0 4.47926 4.45588 0.0380859 9.97059 0.0380859C15.4853 0.0380859 20 4.47926 20 10.0087ZM9.97059 17.6263C14.1618 17.6263 17.6176 14.1999 17.6176 10.0087C17.6176 5.8175 14.1618 2.42044 9.97059 2.42044C5.77941 2.42044 2.39706 5.8175 2.39706 10.0087C2.39706 14.1999 5.77941 17.6263 9.97059 17.6263Z" fill="#274769"/>
                            <path fillRule="evenodd" clipRule="evenodd" d="M16.4853 2.43515C14.7353 0.935145 12.4706 0.0380859 9.98529 0.0380859C4.47059 0.0380859 0 4.50868 0 10.0381C0 12.4499 0.852941 14.6557 2.26471 16.3763C2.27941 16.6557 2.17647 17.2587 1.79412 17.8469C1.5 18.244 1.33824 18.391 1.23529 18.494C1.14706 18.5675 1.11765 18.6116 1.08824 18.6999C1.07353 18.7293 1.07353 18.9204 1.27941 19.0528C1.52941 19.2146 1.98529 19.2734 2.92647 19.1263C3.19118 19.0528 3.88235 18.8616 4.44118 18.7293C4.54412 18.6999 4.79412 18.641 4.92647 18.6704C6.41177 19.5528 8.14706 20.0528 9.98529 20.0528C12.6471 20.0528 15.0735 19.0087 16.8676 17.3028C17 17.1263 17.0294 16.8763 17.0441 16.8028C17.0735 16.1116 16.3971 15.7881 16.2647 15.7293C15.8529 15.5381 15.1471 15.6999 14.8824 15.8469C13.5588 16.9646 11.8529 17.641 9.98529 17.641C5.79412 17.641 2.41176 14.244 2.41176 10.0528C2.41176 5.86162 5.80882 2.46456 9.98529 2.46456C11.6765 2.46456 13.25 3.02338 14.5147 3.96456L14.5 3.94985C14.6176 4.03809 15.0147 4.28809 15.7353 4.00868C16.0588 3.89103 16.5882 3.46456 16.6471 3.18515C16.6912 2.92044 16.5735 2.61162 16.4853 2.43515Z" fill="#3399FF"/>
                            <path d="M13.6032 11.435C14.3826 11.435 15.0297 10.8026 15.0297 10.0085C15.0297 9.21438 14.3973 8.58203 13.6032 8.58203C12.8091 8.58203 12.1768 9.21438 12.1768 10.0085C12.1768 10.8026 12.8238 11.435 13.6032 11.435Z" fill="#59A2F6"/>
                            <path d="M9.97139 11.435C10.7508 11.435 11.3979 10.8026 11.3979 10.0085C11.3979 9.21438 10.7655 8.58203 9.97139 8.58203C9.17728 8.58203 8.54492 9.21438 8.54492 10.0085C8.54492 10.8026 9.17728 11.435 9.97139 11.435Z" fill="#4480C2"/>
                            <path d="M6.33858 11.435C7.11799 11.435 7.76505 10.8026 7.76505 10.0085C7.76505 9.21438 7.1327 8.58203 6.33858 8.58203C5.54446 8.58203 4.91211 9.21438 4.91211 10.0085C4.91211 10.7879 5.54446 11.435 6.33858 11.435Z" fill="#375D86"/>
                        </svg>
                        <h4 className="sidebar-title">Clerkie Challenge</h4>
                    </div>
                </div>
                <div className="sidebar-body">{renderSidebarButtons()}</div>
        </div>
        </>
    )
}

export default Navigation;